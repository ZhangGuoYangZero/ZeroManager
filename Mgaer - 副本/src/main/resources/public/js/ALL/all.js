layui.use(['table', 'layer'], function () {
    ...
    var tableIns = table.render({
        ...
        , cols: [[ //表头
            ...
            , {field: 'model', title: '型号', align: 'center'}
            , {
                field: 'statu', title: '状态', align: 'center', templet: function (statut) {
                    //console.log(statut);
                    return formatColor(statut.statu);
                }
            }
            , {field: 'indata', title: '入仓时间', align: 'center'}
            , {title: '操作', templet: "#supplyBar", fixed: 'right', align: 'center', minwidth: 150}
        ]]
    });

    //返回的HTML语句  要用这个“” 包起来
    function formatColor(statu) {
        if (statu == 0) {
            return "<div style='color:grey'>须维护</div>";
        } else {
            return "<div style='color:green'>可使用</div>";
        }
    }
    ...
    $('.search_btn').click(function () {
        ...
        tableIns.reload({
            ...
            where: {
                DeviceName: $("[name = 'supplyName']").val()//提供商
