layui.use(['table', 'layer'], function () {
    ...
    var tableIns = table.render({
        ...
        , cols: [[ //表头
            ...
            , {field: 'supply', title: '供应商', sort: true, align: 'center'}
            , {field: 'deviceName', title: '设备名', align: 'center'}
            , {field: 'model', title: '型号', align: 'center'}
            , {field: 'numberOfDevices', title: '数量', align: 'center'}
            , {
                field: 'isSelected', title: '选定', align: 'center', templet: function (statut) {
                    //console.log(statut);
                    return formatColor(statut.isSelected);
                }
            }
    ...
    function formatColor(isSelected) {
        if (isSelected == 0) {
            return "<div style='color:grey'>不能供货</div>";
        } else {
            return "<div style='color:green'>可以供货</div>";
        }
    }
    ...
    $('.search_btn').click(function () {
        ...
        tableIns.reload({
            ...
            where: {
                Supply: $("[name = 'supplyName']").val()//提供商
    ...
    function RefNewWintoAdd(iD) {
        var url = ctx + '/suppliers/addorDel';
        var title = 'addSupply';
        if (iD != null && iD != '') {
            title = "edidSupply"
            url += '?iD=' + iD;
